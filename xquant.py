...
def our_quantize_q3k(x: np.ndarray) -> np.ndarray:
    ...
    # 🔴 2026-08-01: ВЗВЕШЕННЫЙ ПОИСК ТЕПЕРЬ ПО УМОЛЧАНИЮ. Прежнее решение («наивный лучше»)
    # было принято 04.07 по ОБЩЕЙ ошибке: 18.75% naive против 19.90% weighted. Цифра верная,
    # но она врёт на Q3_K — набирается на почти-нулях, которых много и которых глаз не видит.
    # Замер по группам на боевом слое FLUX (без imatrix) показал обратное:
    #                 общая     СМЕЩЕНИЕ      КРУПНЫЕ |w|>2σ
    #   наивный      0.1881   +4.914e-04       0.1504
    #   взвешенный   0.1995   −7.366e-06       0.1035
    # Смещение (систематический увод тона — та самая «пелена») упало В 67 РАЗ, до уровня
    # здоровых Q2_K/Q4_0. Ошибка на КРУПНЫХ весах — на 31% меньше, и это НИЖЕ Q2_K (0.1445):
    # трёхбитный наконец лучше двухбитного там, где видно глазу. Раньше было наоборот —
    # больше бит давали худшую картинку, и это была наша единственная аномалия с 04.07.
    # Общая ошибка при этом чуть выросла (+6%) — и именно за неё мы три недели держали
    # худший вариант. Формат не тронут, совместимость с UnetLoaderGGUF цела.
    # ОТКАТ: XQUANT_NAIVE_Q3=1 (вернёт absmax/4).
    _naive_q3 = os.environ.get("XQUANT_NAIVE_Q3", "0").strip().lower() in ("1", "on", "true", "yes")
    if not _naive_q3:
