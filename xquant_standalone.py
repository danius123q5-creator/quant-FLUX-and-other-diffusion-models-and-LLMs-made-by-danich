...
def _type_weight(key):
    ...
    вскрытие боевого Q2_K показало, что llama.cpp жмёт attention СИЛЬНЕЕ, а ffn бережёт,
    то есть ровно наоборот. Кто прав — решает рендер, а не мы.
    """
...
def _smart_alloc_v2(items, qn, mode="balance", log=print):
    ...
    valid = [(k, c, p, s * _tw(k)) for (k, c, p, s) in valid]
