# Жматель (gears2) — свой квантайзер диффузионных моделей (без чужого движка)
...
```
gears2.exe model-Q4_K_M.gguf Q3_K     # готовый LM-Studio квант → меньше, токенайзер цел
```
...
Не каждый слой заслуживает одинаковой битности. Перед записью gears2 прогоняет
...
2. **Нода ComfyUI** — поставь **gears2 imatrix: Capture** перед `KSampler` и **Save**
   после, прогони 1–3 генерации → `<model>.imatrix.npy`.

Укажи файл в `gears2.exe` (поле 🎯 imatrix) или `XQUANT_IMATRIX=<путь>`. Композится с
...
## Использование
**Проще всего — standalone `gears2.exe`** (20 МБ, Python не нужен, numpy внутри):
перетащи `.safetensors`-модель на `gears2.exe` → получишь `<модель>-Q2_K.gguf` рядом.
Или из терминала: `gears2.exe <model.safetensors> [Q4_0|Q3_K|Q2_K]`.
...
**`gears2 GGUF Loader`** — выбираешь `.gguf`, он разжимает при загрузке и строит модель
...
- **Аудио-диффузия в `.safetensors`** (Stable Audio, ACE-Step) → Жматель сожмёт её
  (тег арх `unknown`), а нода **`gears2 Music Loader`** загрузит обратно. ComfyUI сам
